...
class TraineeTab(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_trainee_delete.clicked.connect(self.trainee_delete)
    ...
    def trainee_insert(self):
        if (self.edit_sd_num.text() != "") and (self.edit_t_section.text() != "") and (self.edit_l_num.text() != "") and (self.edit_l_division.text() != ""):
            try:
                ...
                QMessageBox.information(self, "등록 성공", "등록 되었습니다.",
                                        QMessageBox.Yes, QMessageBox.Yes)

            except:
                QMessageBox.information(self, "등록 오류", "올바른 형식으로 입력하세요.",
                                        QMessageBox.Yes, QMessageBox.Yes)
                return
        else:
            QMessageBox.information(self, "입력 오류", "빈칸 없이 입력하세요.",
    ...
    def trainee_edit(self):
        if (self.edit_sd_num.text() != "") and (self.edit_t_section.text() != "") and (
                self.edit_l_num.text() != "") and (self.edit_l_division.text() != ""):
            try:
                self.lesson_list = "update trainee set l_num = '{}', t_section = '{}' where t_no = '{}'" \
                .format(self.edit_l_num.text(), self.edit_t_section.text(), self.edit_t_no.text())
                self.uni_cur.execute(self.lesson_list)
                self.conn.commit()
                QMessageBox.information(self, "수정 성공", "수정 되었습니다.",
                                        QMessageBox.Yes, QMessageBox.Yes)
    ...
    def trainee_delete(self):
        if (self.edit_t_no.text() != ""):
            try:
                agree = QMessageBox.question(self, "취소 확인", "정말로 취소 하시겠습니까?",
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if agree == QMessageBox.Yes:
                    self.t_delete_sql = "delete from trainee where t_no = '{}'" \
                        .format(self.edit_t_no.text())
                    print(self.t_delete_sql)

                    self.uni_cur.execute(self.t_delete_sql)
                    self.conn.commit()



            except:
                QMessageBox.information(self, "취소 오류", "잘못 선택된 회원입니다.",
                                        QMessageBox.Yes, QMessageBox.Yes)
                return
        else:
            QMessageBox.information(self, "취소 오류", "회원 번호가 없습니다.",
                                    QMessageBox.Yes, QMessageBox.Yes)
            return

        QMessageBox.information(self, "취소 성공", "취소되었습니다.",
                                QMessageBox.Yes, QMessageBox.Yes)
        self.trainee_init()
        self.traineeList(self)
    ...
    def traineeList(self, QShowEvent):
        self.trainee_sql = "SELECT t.t_no, t.sd_num, t.l_num, l.l_name, t.t_section, t.t_date FROM trainee t, lesson l WHERE l.l_num = t.l_num"
        self.uni_cur.execute(self.trainee_sql)
        self.conn.commit()
        rs = self.uni_cur.fetchall()

        self.trainee_item_list.removeRow(len(rs))
        ...
        for i in range(len(rs)):
            ...
            self.trainee_item_list.setData(self.trainee_item_list.index(i, 5), str(rs[i][5]))
